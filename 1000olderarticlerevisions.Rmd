...
library(zoo)
# p <-  ggplot(data = collapsed, 
#        aes(x=as.yearmon(date), y = count, colour = article_id)) +       
...
ggsave("img/article-revisions-by-year-date.png", p)
```
...
# USE THIS
ggplot(data = collapsed.by.day, aes(x=as.Date(date))) +
  geom_line(aes(y = count), color = "blue") +
  geom_line(aes(y=unique.articles), color = "lightblue") +
  labs(title = 'Behavior of Articles First Revised in October 2002') +
  xlab("Days") +
  ylab("Number of Revisions") +
  # scale_color_manual(name = "Colors", 
  #                    values = c("blue" = "blue", "lightblue" = "lightblue"))
...
data.count.indicators[which(data.count.indicators$article_id == 1073), "Indicator"] <- "Deleted Article"
...
ggplot(data = data.count.indicators[which(!is.na(data.count.indicators$Indicator) & data.count.indicators$Indicator == "Deleted Article"),], aes(x=as.yearmon(date), y = count, colour = Indicator)) + geom_line(aes(group = article_id), size = .8, alpha = 0.6) + geom_point(size = .75)
```
