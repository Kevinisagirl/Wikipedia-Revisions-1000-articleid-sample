...
for (f in factorlevels){
  print(ggplot(data = df1_5yrs[df1_5yrs$bins == f,], aes(x=variable, y = value)) + geom_line(aes(group = article_id), color=colorsused[counter], size = 0.5) + scale_y_continuous(labels = scales::percent) + ylab("Percent of Article Revisions") + xlab("Months since First Revision") + labs(color = "Total Article Revisions") + facet_grid(~bins) + theme(legend.position="none"))
  counter <- counter +1
}
```
